...
var Application = module.exports = function() {
	...
	self.frexAPIManager = new APIManager(self);
	self.frexRemote = new Remote(self);

	var frexApp = {
		Engine: function(engineName) {
			var runtime = self.frexEngine.getRuntime(engineName);
...
Application.prototype.listen = function() {
	...
	var tasks = [
		function(complete) {
			...
			self.frexEngine.init(function() {
				complete();
			});
		},
		function(complete) {
			// Initializing API manager
			self.frexAPIManager.init(function() {
				complete();
			});
		},
		function(complete) {
			// Initializing Remote handler
			self.frexRemote.init(function() {
				complete();
			});
		},
		function(complete) {
