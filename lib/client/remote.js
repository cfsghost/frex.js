...
Frex.Remote.prototype.commit = function(completed) {
    ...
    if (Object.keys(self._connectionParams).length > 0) {
        params = JSON.parse((JSON.stringify(commit_object) + JSON.stringify(self._connectionParams)).replace(/}{/g,','));
    } else {
        params = commit_object;
    }
...
Frex.Remote.prototype.makeEventConnection = function(completed) {
    ...
    if (Object.keys(self._connectionParams).length > 0) {
        params = JSON.parse((JSON.stringify(event_object) + JSON.stringify(self._connectionParams)).replace(/}{/g,','));
    } else {
        params = event_object;
    }
